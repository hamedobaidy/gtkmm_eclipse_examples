...
MyRanges::MyRanges() :
		m_VBox_Top(
				Gtk::ORIENTATION_VERTICAL, 0),
		m_VBox2(
				Gtk::ORIENTATION_VERTICAL, 20),
		m_VBox_HScale(
				Gtk::ORIENTATION_VERTICAL, 10),
		m_HBox_Scales(
		...
		m_adjustment_digits(
				Gtk::Adjustment::create(1.0, 0.0, 5.0, 1.0, 2.0)),
		m_adjustment_pagesize(
				Gtk::Adjustment::create(1.0, 1.0, 101.0)),
	...
	m_CheckButton.signal_toggled().connect( sigc::mem_fun(*this,
	   &MyRanges::on_checkbutton_toggled) );
	...
	m_ComboBox_Position.signal_changed().connect(
			sigc::mem_fun(*this, &MyRanges::on_combo_position) );
	...
	m_adjustment_digits->signal_value_changed().connect(sigc::mem_fun(*this,
	    &MyRanges::on_adjustment1_value_changed));
	...
	m_adjustment_pagesize->signal_value_changed().connect(sigc::mem_fun(*this,
	    &MyRanges::on_adjustment2_value_changed));
